...
PredictionMap.585.126.2040 <- tm_shape(prediction.2040.585.126) +
  ...
  tm_layout(
    inner.margins = c(margin, margin, margin, margin),
    title.size = title_size, legend.position = NULL,legend.show = F,
    legend.title.size = legend_title_size,
    legend.text.size = legend_title_size * 0.75,
    main.title = "From SSP1-2.6 to SSP 5-8.5", 
    main.title.position = "center",
    main.title.size = 0.8
  ) 
# figure S25
...
PredictionMap.585.126.2100 <- tm_shape(prediction.2100.585.126) +
  ...
  tm_layout(
    ...
    legend.title.size = legend_title_size * 0.4,
    legend.text.size = legend_title_size * 0.4
  ) 
# figure S31
...
multi.panel <-
  tmap_arrange(PredictionMap.245.126.2040, PredictionMap.370.126.2040, PredictionMap.585.126.2040,
               ...
               ncol = 3, nrow = 3, widths = c(.36, .32, .32), heights = c(.36, .32, .32))
multi.panel %>%
  tmap_save(filename = "06_Figure/S41_ICC_multiPanel.jpg", width = 300, height = 140, units = 'mm', dpi = 1000)
